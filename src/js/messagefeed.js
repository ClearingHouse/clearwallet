...
function MessageFeed() {
  ...
  self.sellBTCOrdersCount = ko.computed(function() {
    return $.map(self.OPEN_ORDERS, function(item) {       
        return ('VIA' == item['get_asset']) ? item : null;
    }).length;
  }, self);
      ...
      function(data, endpoint) {
        ...
        var openBTCOrdersToCancel = $.grep(data, function(e) {
          return    e['status'] == 'open'
                 && (e['get_asset'] == 'VIA' || e['give_asset'] == 'VIA')
  ...
  self.parseMempoolTransaction = function(txHash, category, message) {
    ...
    if (displayTx) {
      WALLET.searchDivisibility(message['bindings']['asset'] || 'VIA', function(divisibility) {
  ...
  self.handleMessage = function(txHash, category, message) {
    ...
    if(category == "balances") {
      ...
      if(WALLET.getAddressObj(message['address'])) {
        ...
        if(message['_balance'] == 0 && message['asset'] != "VIA" && message['asset'] != "XCH") {
      ...
      if(match) {
        if(message['_status'] != 'open') { //order is filled, expired, or cancelled, remove it from the listing
          ...
          //if the order is for BTC and the qty remaining on either side is negative (but not on BOTH sides,
          // as it would be fully satified then and canceling would be pointless), auto cancel the order
          if( (match['get_asset'] == 'VIA' || match['give_asset'] == 'VIA')
      ...
      if ((WALLET.getAddressObj(message['tx0_address']) && message['forward_asset'] == 'VIA' && message['_status'] == 'pending')
         || (WALLET.getAddressObj(message['tx1_address']) && message['backward_asset'] == 'VIA' && message['_status'] == 'pending')) {
      ...
      } else if ((WALLET.getAddressObj(message['tx1_address']) && message['forward_asset'] == 'VIA' && message['_status'] == 'pending')
         || (WALLET.getAddressObj(message['tx0_address']) && message['backward_asset'] == 'VIA' && message['_status'] == 'pending')) {
