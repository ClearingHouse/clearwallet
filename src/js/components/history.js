function BalanceHistoryViewModel() {
  ...
  self.availableAssets = !USE_TESTNET ? ko.observableArray(["XCH", "VIA"]) : ko.observableArray(["XCH"]);
  ...
  self.assetChanged = function() {
    ...
    if(self.selectedAsset() == "VIA") { //mainnet only (as we use blockchain.info for this and they don't support testnet)
      ...
      for(var i=0; i < addresses.length; i++) {
        ...
        $.queryYQL(q, function(data, textStatus, jqXHR) {
          ...
          var address = /address%3D([A-Za-z0-9]+)%22/g.exec(jqXHR.url)[1];
          var addressHash = hashToB64(address);
          $.jqlog.debug( "Could not get VIA balance from blockchain for address " + address + ": " + errorThrown);
          var addressName = PREFERENCES['address_aliases'][addressHash] ? "<b>" + PREFERENCES['address_aliases'][addressHash] + "</b> (" + address + ")" : address; 
...
function TransactionHistoryItemViewModel(data) {
  ...
  self.dispDescription = function() {
    ...
    if(self.RAW_TX_TYPE == 'burns') {
      desc = "XCH Proof-of-Burn<br/>Burned: <Am>" + normalizeQuantity(self.DATA['burned']) + "</Am> <As>VIA</As><br/>"
        + "Earned: <Am>" + smartFormat(normalizeQuantity(self.DATA['earned']) ) + "</Am> <As>XCH</As>";
      ...
      if(self.DATA['forward_asset'] == 'VIA' || self.DATA['backward_asset'] == 'VIA') {
        desc += " <b>(pending VIApay)</b>";
      }
    } else if(self.RAW_TX_TYPE == 'btcpays') {
      desc = "Payment for <Am>" + smartFormat(normalizeQuantity(self.DATA['btc_amount'])) + "</Am> <As>VIA</As>";
      ...
      desc = BET_TYPES[self.DATA['bet_type']] + " bet on feed @ <Ad>"
        ...
        + smartFormat(normalizeQuantity(self.DATA['wager_quantity'])) + "</Am> <As>XCH</As>, Counterwager: <Am>"
        + smartFormat(normalizeQuantity(self.DATA['counterwager_quantity'])) + "</Am> <As>XCH</As>";
      ...
      desc = "For feed @ <Ad>" 
        ...
        + smartFormat(normalizeQuantity(self.DATA['forward_quantity'])) + "</Am> <As>XCH</As> and <Ad>"
        + getAddressLabel(self.DATA['tx1_address']) + "</Ad> bet <Am>"
        + smartFormat(normalizeQuantity(self.DATA['backward_quantity'])) + "</Am> <As>XCH</As>";
      ...
      desc = "Rock-Paper-Scissors bet, Wager: <Am>"
        + smartFormat(normalizeQuantity(self.DATA['wager'])) + "</Am> <As>XCH</As>";
