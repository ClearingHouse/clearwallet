...
function SendModalViewModel() {
  ...
  self.show = function(fromAddress, asset, rawBalance, isDivisible, resetForm) {
    if(asset == 'VIA' && rawBalance == null) {
      return bootbox.alert("Cannot send <b class='notoAssetColor'>VIA</b> right now, as we cannot currently get"
...
function SweepModalViewModel() {
      ...
      validator: function (val, self, callback) {
        ...
        if  (totalBtcBalanceForSweeep < sweepingCost) {
          ...
          this.message = "We're not able to sweep all of the tokens you selected. Please send "
                        ...
                        + " VIA transactions to address " + self.addressForPrivateKey() + " and try again."
  ...
  self.addressForPrivateKey.subscribe(function(address) {
    ...
    self.sweepAssetsCost = {'VIA': MIN_FEE+REGULAR_DUST_SIZE};
      ...
      failoverAPI("get_asset_info", {'assets': assets}, function(assetsData, endpoint) {
        ...
        WALLET.retriveBTCAddrsInfo([address], function(data) {
          ...
          if(unconfirmedRawBal > 0) {
            ...
            var viewModel = new SweepAssetInDropdownItemModel("VIA", unconfirmedRawBal, normalizeQuantity(unconfirmedRawBal));
  ...
  self.addressForPrivateKeyForFees.subscribe(function(address) {
    ...
    WALLET.retriveBTCAddrsInfo([address], function(data) {
      $.jqlog.debug(data);
      self.addressForFeesBalanceMessage(normalizeQuantity(data[0]['confirmedRawBal'])+' VIA in '+address);
      ...
      WALLET.BITCOIN_WALLET.getOldAddressesInfos(function(data) {
        for (var address in data) {
          if (self.excludedOldAddresses.indexOf(address) == -1) {
            self.availableOldAddresses.push(new BalancesAddressInDropdownItemModel(address, address, data[address]['VIA']['privkey']));
          }       
        }
      }); 
    }    
  ...
  self.sendBtcForFees = function(callback) {
    ...
    for(var i = 0; i < self.selectedAssetsToSweep().length; i++) {
      var assetName = self.selectedAssetsToSweep()[i];
      if (assetName=='VIA') sweepBTC = true;
    }
    if (sweepBTC) {
    ...
    var message = "Sending " + normalizeQuantity(self.missingBtcForFees) + " VIA from "
  ...
  self.mergeOutputs = function(key, pubkey, callback, fees) {
    if (self.txoutsCountForPrivateKey>1) {
      ...
      var onTransactionError = function() {
        if (arguments.length==4) {
          var match = arguments[1].match(/Insufficient viacoins at address [^\s]+\. \(Need approximately ([\d]+\.[\d]+) VIA/);
          if (match!=null) {
    ...
    multiAPIConsensus("create_issuance", transferData,
      function(unsignedTxHex, numTotalEndpoints, numConsensusEndpoints) {
        ...
        WALLET.broadcastSignedTx(signedHex, function(issuanceTxHash, endpoint) { //broadcast was successful
          ...
          $.jqlog.debug("New VIA balance: "+newBtcBalance);
  ...
  self._doSendAsset = function(asset, key, pubkey, opsComplete, adjustedBTCQuantity, callback) {
    ...
    //TODO: remove this
    if(asset == 'VIA') assert(adjustedBTCQuantity !== null);
    ...
    var quantity = (asset == 'VIA') ? (self.btcBalanceForPrivateKey() - MIN_FEE) : selectedAsset.RAW_BALANCE;
    var normalizedQuantity = (asset == 'VIA') ? normalizeQuantity(quantity) : selectedAsset.NORMALIZED_BALANCE;
    
    assert(selectedAsset);
    
    if(!quantity) { //if there is no quantity to send for the asset, only do the transfer
      if(asset == 'XCH' || asset == 'VIA') { //nothing to send, and no transfer to do
    ...
    multiAPIConsensus("create_send", sendData, //can send both BTC and counterparty assets
      function(unsignedTxHex, numTotalEndpoints, numConsensusEndpoints) {
        ...
        WALLET.broadcastSignedTx(signedHex, function(sendTxHash, endpoint) { //broadcast was successful
          ...
          if (selectedAsset.ASSET != 'XCH'
             && selectedAsset.ASSET != 'VIA'
  ...
  self.doAction = function() {
    ...
    if(hasBTC !== false) {
      ...
      sendsToMake.push(["VIA", cwk, pubkey, opsComplete, self.btcBalanceForPrivateKey()]);
    }
    ...
    var launchSweep = function() {
      if (sendsToMake.length==1 && sendsToMake[0][0]=='VIA') {
        doSweep();
      } else {
...
function TestnetBurnModalViewModel() {
  ...
  self.btcBurnQuantity = ko.observable('').extend({
    ...
    validation: [{
      ...
      message: 'Quantity entered must be between 0 and 20000 VIA.',
      params: self
    },{
      validator: function (val, self) {
        return parseFloat(val) <= WALLET.getBalance(self.address(), 'VIA') - normalizeQuantity(MIN_FEE);
      },
      message: 'The quantity of VIA entered exceeds your available balance.',
      params: self
    },{
      validator: function (val, self) {
        return !(parseFloat(val) > 20000 - self.btcAlreadyBurned());
      },
      message: 'You can only burn <b>20000 VIA</b> total for any given address. Even over multiple burns, the total quantity must be less than <b>20000 VIA</b>.',
      params: self
    }]
  });
  ...
  self.maxPossibleBurn = ko.computed(function() { //normalized
    ...
    return Math.min(20000 - self.btcAlreadyBurned(), WALLET.getAddressObj(self.address()).getAssetObj('VIA').normalizedBalance())
  }, self);
    ...
    WALLET.doTransaction(self.address(), "create_burn",
      ...
      function(txHash, data, endpoint, addressType, armoryUTx) {
        ...
        var message = "You " + (armoryUTx ? "will be burning" : "have burned") + " <b class='notoQuantityColor'>" + self.btcBurnQuantity() + "</b>"
          + " <b class='notoAssetColor'>VIA</b> for approximately"
          + " <b class='notoQuantityColor'>" + self.quantityXCPToBeCreated() + "</b>"
          + " <b class='notoAssetColor'>XCH</b>. ";
        WALLET.showTransactionCompleteDialog(message + ACTION_PENDING_NOTICE, message, armoryUTx);
      }
    );
