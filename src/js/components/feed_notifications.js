...
NotificationViewModel.calcText = function(category, message) {
  ...
  if(category == "sends") {
    ...
    desc = "VIAPay from <Ad>" + getAddressLabel(message['source']) + "</Ad> to <Ad>" + getAddressLabel(message['destination'])
      + "</Ad> for <Am>" + smartFormat(normalizeQuantity(message['btc_amount'])) + "</Am> <As>VIA</As>";
    ...
    desc = "Your address <Ad>" + getAddressLabel(message['source']) + "</Ad> has burned <Am>" + smartFormat(normalizeQuantity(message['burned']))
      + "</Am> <As>VIA</As> for <Am>" + smartFormat(normalizeQuantity(message['earned'])) + "</Am> <As>XCH</As>";
    ...
    if(category == "callbacks") {
      desc = "<As>XCH</As> balance adjusted on your address(es) <Ad>" + addressesWithAsset.join(', ')
    ...
    desc = "You bet <Am>" + smartFormat(normalizeQuantity(message['wager_quantity'])) + "</Am> <As>XCH</As> on the feed @"
    ...
    desc = "Bet @ feed <Ad>" + message['feed_address'] + "</Ad> matched between <Ad>" 
      + getAddressLabel(message['tx0_address']) + "</Ad> (gave <Am>"
      + smartFormat(normalizeQuantity(message['forward_quantity'])) + "</Ad> <As>XCH</As>) and <Ad>"
      + getAddressLabel(message['tx1_address']) + "</Ad> (gave <Am>"
      + smartFormat(normalizeQuantity(message['backward_quantity'])) + "</Ad> <As>XCH</As>)";
    ...
    desc  = "You play Rock-Paper-Scissors with <Am>" + smartFormat(normalizeQuantity(message['wager']))+ "</Am> <As>XCH</As>";
      ...
      failoverAPI('get_rps_matches', params, onReceiveRpsMatch);
        ...
        if (WALLET.getAddressObj(message['tx0_address'])) {
          if  (message['status'] == "concluded: first player wins") {
            ...
            desc = "RPS: You lose " + smartFormat(normalizeQuantity(message['wager']))+ "</Am> <As>XCH</As>" +
                   " with <Ad>" + getAddressLabel(message['tx0_address']) + "</Ad>";
        ...
        if (WALLET.getAddressObj(message['tx1_address'])) {
          if  (message['status'] == "concluded: first player wins") {
            desc2 = "RPS: You lose " + smartFormat(normalizeQuantity(message['wager']))+ "</Am> <As>XCH</As>" +
                   " with <Ad>" + getAddressLabel(message['tx1_address']) + "</Ad>";
          } else if  (message['status'] == "concluded: second player wins") {
            desc2 = "RPS: You win " + smartFormat(normalizeQuantity(message['wager']))+ "</Am> <As>XCH</As>" +
                   " with <Ad>" + getAddressLabel(message['tx1_address']) + "</Ad>";
          } else if  (message['status'] == "concluded: tie") {
