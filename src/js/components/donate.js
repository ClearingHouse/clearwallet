...
function DonationViewModel() {
  ...
  self.donationCurrency = ko.observable('VIA');
  ...
  self.init = function() {
    ...
    for(var i = 0; i < addresses.length; i++) {
      var btcBalance = WALLET.getBalance(addresses[i][0], 'VIA', true);
      options.push({
        address: addresses[i][0], 
        label: addresses[i][1] + ' (' + round(btcBalance, 2) + ' VIA / ' + round(addresses[i][2], 2) + ' XCH)'
      });
